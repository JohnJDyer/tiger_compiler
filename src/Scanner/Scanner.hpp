...
class Scanner {
 ...
 private:
  void error(std::string message) {
    numErrors++;
    std::cout << globalFileName << ":" << currLine << ": Lexical error: "
              << message << "\n";
  }
 ...
 public:
  Scanner(std::string fileName) {
    ...
    std::string tmparr[] = { "array", "break", "do", "else", "end", "for",
        "function", "if", "in", "let", "of", "then", "to", "type", "var",
        "while", "endif", "begin", "enddo", "return", "int", "float" };
    keywords = std::set<std::string>(std::begin(tmparr), std::end(tmparr));
    keywordsVector = std::vector<std::string>(std::begin(tmparr),
                                              std::end(tmparr));
  }
 private:
  /**
 ...
 public:
  TokenPair getToken() {
    while (true) {
      ...
      if (currChar == -1) {
        return TokenPair(Symbol::Terminal::EOFF, "");
      }
      ...
      if (isalpha((char) currChar)) {
        ...
        if (keywords.count(word)) {
          ...
          return TokenPair(terminalEnum, word);
        }
        // Else it is an identifier
        return TokenPair(Symbol::Terminal::ID, word);
      }
      ...
      if (isdigit((char) currChar)) {
        ...
        if ((char) currChar == '.') {
          ...
          return TokenPair(Symbol::Terminal::FLOATLIT,
                           sNumber + "." + sDecimals);
        }
        // Else it is just an int
        unread(currChar);
        return TokenPair(Symbol::Terminal::INTLIT, sNumber);
      }
      ...
      switch ((char) currChar) {
        ...
        case ',':
          return TokenPair(Symbol::Terminal::COMMA, ",");
        case ';':
          return TokenPair(Symbol::Terminal::SEMI, ";");
        case '(':
          return TokenPair(Symbol::Terminal::LPAREN, "(");
        case ')':
          return TokenPair(Symbol::Terminal::RPAREN, ")");
        case '[':
          return TokenPair(Symbol::Terminal::LBRACK, "[");
        case ']':
          return TokenPair(Symbol::Terminal::RBRACK, "]");
        case '{':
          return TokenPair(Symbol::Terminal::LBRACE, "{");
        case '}':
          return TokenPair(Symbol::Terminal::RBRACE, "}");
        case '.':
          return TokenPair(Symbol::Terminal::PERIOD, ".");
        case '+':
          return TokenPair(Symbol::Terminal::PLUS, "+");
        case '-':
          return TokenPair(Symbol::Terminal::MINUS, "-");
        case '*':
          return TokenPair(Symbol::Terminal::MULT, "*");
        case '&':
          return TokenPair(Symbol::Terminal::AND, "&");
        case '|':
          return TokenPair(Symbol::Terminal::OR, "|");
        case '=':
          return TokenPair(Symbol::Terminal::EQ, "=");
        case '_':
          unread(currChar);
        ...
        case ':':
          nextChar = read();
          if ((char) nextChar == '=') {
            return TokenPair(Symbol::Terminal::ASSIGN, ":=");
          }
          unread(nextChar);
          return TokenPair(Symbol::Terminal::COLON, ":");
        case '<':
          nextChar = read();
          switch ((char) nextChar) {
            case '>':
              return TokenPair(Symbol::Terminal::NEQ, "<>");
            case '=':
              return TokenPair(Symbol::Terminal::LESSEREQ, "<=");
            default:
              unread(nextChar);
              return TokenPair(Symbol::Terminal::LESSER, "<");
          }
        case '>':
          nextChar = read();
          if ((char) nextChar == '=') {
            return TokenPair(Symbol::Terminal::GREATEREQ, ">=");
          }
          unread(nextChar);
          return TokenPair(Symbol::Terminal::GREATER, ">");
        case '/':
          nextChar = read();
          if ((char) nextChar == '*') {
          ...
          unread(nextChar);
          return TokenPair(Symbol::Terminal::DIV, "/");
        default:
